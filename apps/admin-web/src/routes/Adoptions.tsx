...
export function Adoptions() {
  ...
  const filtered = useMemo(() => {
    let list = adoptions;
    const pet = filterPet.trim().toLowerCase();
    const tutor = filterTutor.trim().toLowerCase();
    if (pet) list = list.filter((a) => (a.petName ?? '').toLowerCase().includes(pet));
    if (tutor) list = list.filter((a) => (a.tutorName ?? '').toLowerCase().includes(tutor));
    return list;
  }, [adoptions, filterPet, filterTutor]);
        ...
        <>
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <input
              type="text"
              value={filterPet}
              onChange={(e) => { setFilterPet(e.target.value); setPage(1); }}
            ...
            <input
              type="text"
              value={filterTutor}
              onChange={(e) => { setFilterTutor(e.target.value); setPage(1); }}
          ...
          <div className="mt-2 px-1 border-t border-adopet-primary/10 pt-2">
            <PaginationBar
              page={pageSafe}
              pageSize={pageSize}
              total={total}
