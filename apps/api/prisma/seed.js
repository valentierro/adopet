...
const NEIGHBORHOOD_DELTA = 0.005;
const CIDADES_PE = [
...
function seedPhotoUrl(folder, index) {
...
async function seedPetsForExistingUsers() {
    ...
    const pickPartnerId = () => partnerIds.length > 0 && Math.random() < SEED_PET_PARTNER_CHANCE ? pick(partnerIds) : undefined;
    console.log(`Encontrados ${users.length} usuário(s). Criando ${SEED_DOGS_PER_USER} cachorros e ${SEED_CATS_PER_USER} gatos para cada (bairros: Recife/Jaboatão)...`);
    for (const user of users) {
        const usedDogNames = new Set();
        const usedCatNames = new Set();
        for (let i = 0; i < SEED_DOGS_PER_USER; i++) {
            let name = pick(DOG_NAMES);
            while (usedDogNames.has(name))
                name = pick(DOG_NAMES);
...
const SEED_PARTNERS_ONG = [
    {
        slug: 'instituto-amor-de-patas',
        name: 'Instituto Amor de Patas',
        city: 'Recife',
        description: 'ONG dedicada ao resgate e adoção responsável de cães e gatos na região metropolitana do Recife. Atuamos com castração, vacinação e divulgação de anúncios em parceria com o Adopet.',
    ...
    },
    {
        ...
        description: 'Projeto de proteção animal que promove adoções e eventos de conscientização. Parceiros do Adopet para divulgar pets que precisam de um lar.',
    ...
    },
    {
        ...
        city: 'Caruaru',
        description: 'Rede de lares temporários e adoção de cães e gatos no agreste pernambucano. Trabalhamos em parceria com o Adopet para ampliar a divulgação dos nossos resgatados.',
        website: 'https://lartemporariope.com.br',
        email: 'contato@lartemporariope.com.br',
        phone: '(81) 97777-0003',
    },
];
const SEED_PARTNERS_COMERCIAL = [
    {
        type: 'CLINIC',
