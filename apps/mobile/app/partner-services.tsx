...
import { View, Text, TouchableOpacity, FlatList, StyleSheet, Alert, Image, Modal, Pressable, ActivityIndicator } from 'react-native';
...
export default function PartnerServicesScreen() {
  ...
  const [deleteConfirmItem, setDeleteConfirmItem] = useState<PartnerService | null>(null);
  ...
  const confirmDelete = (item: PartnerService) => setDeleteConfirmItem(item);
  ...
  const handleConfirmBatchDelete = async () => {
    ...
    try {
      ...
      showToast(ids.length === 1 ? 'Serviço excluído' : 'Serviços excluídos');
    } catch (e) {
    ...
    <ScreenContainer scroll={false}>
      ...
      <Modal visible={deleteConfirmItem != null} transparent animationType="fade">
        <Pressable style={styles.deleteModalOverlay} onPress={() => setDeleteConfirmItem(null)}>
          <Pressable style={[styles.deleteModalCard, { backgroundColor: colors.surface }]} onPress={(e) => e.stopPropagation()}>
            <Text style={[styles.deleteModalTitle, { color: colors.textPrimary }]}>Excluir serviço?</Text>
            <Text style={[styles.deleteModalMessage, { color: colors.textSecondary }]}>
              {deleteConfirmItem ? `Excluir "${deleteConfirmItem.name}"? Esta ação não pode ser desfeita.` : ''}
            </Text>
      ...
      <Modal visible={batchDeleteConfirmVisible} transparent animationType="fade">
        <Pressable style={styles.deleteModalOverlay} onPress={() => !batchDeleting && setBatchDeleteConfirmVisible(false)}>
          <Pressable style={[styles.deleteModalCard, { backgroundColor: colors.surface }]} onPress={(e) => e.stopPropagation()}>
            <Text style={[styles.deleteModalTitle, { color: colors.textPrimary }]}>Excluir serviços?</Text>
            <Text style={[styles.deleteModalMessage, { color: colors.textSecondary }]}>
              Excluir {selectedIds.size} serviço{selectedIds.size !== 1 ? 's' : ''}? Esta ação não pode ser desfeita.
            </Text>
            <View style={styles.deleteModalActions}>
              <TouchableOpacity
                style={[styles.deleteModalBtn, { borderColor: colors.textSecondary, backgroundColor: 'transparent', marginRight: spacing.sm }]}
