...
const { cellWidth, gap, padding: gridPadding } = gridLayout;
/** Altura da imagem no grid: proporção mais quadrada para card menos cumprido */
...
export default function PartnerMyPetsScreen() {
  ...
  if (loadingPartner || (!partner && !loadingPartner && partnerId === null)) {
    return (
      <ScreenContainer>
        {loadingPartner ? (
      ...
      <ScreenContainer>
        <View style={[styles.filtersWrap, { borderBottomColor: colors.surface }]}>
          ...
          <PageIntro
            ...
            subtitle={partnerName ? `Pets em parceria com ${partnerName}` : 'Pets em parceria com a sua instituição.'}
          />
  ...
  const emptyTitle = 'Nenhum anúncio em parceria';
  const emptyMessage =
    speciesFilter !== 'BOTH'
      ? 'Não há anúncios com esse filtro para a sua ONG.'
      : 'Ainda não há anúncios em parceria com a sua instituição.';
      ...
      return (
        ...
        >
          <Image
            ...
            style={[styles.gridThumb, { width: cellWidth, height: cellWidth / GRID_IMAGE_ASPECT }]}
            contentFit="cover"
  ...
  return (
    <ScreenContainer scroll={false}>
      ...
      <FlashList
        ...
        estimatedItemSize={viewMode === 'grid' ? 180 : 100}
...
const styles = StyleSheet.create({
  ...
  gridCardTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  gridCardName: { fontSize: 13, fontWeight: '700', flex: 1 },
  gridCardMeta: { fontSize: 10, marginTop: 1 },
  gridBadgesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 3, marginTop: 3 },
  gridMatchBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 2,
