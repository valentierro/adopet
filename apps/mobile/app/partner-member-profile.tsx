...
import { getMyPartnerMemberDetails } from '../src/api/partner';
...
export default function PartnerMemberProfileScreen() {
  const { userId, ownerName } = useLocalSearchParams<{ userId: string; ownerName?: string }>();
  const router = useRouter();
  const { colors } = useTheme();

  const { data, isLoading, error } = useQuery({
    queryKey: ['partner-member-details', userId],
    queryFn: () => getMyPartnerMemberDetails(userId!),
    enabled: !!userId,
  });
    ...
    return (
      <ScreenContainer>
        <Text style={[styles.error, { color: colors.textSecondary }]}>
          Não foi possível carregar o perfil do membro.
        </Text>
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer scroll>
      ...
      <Text style={[styles.sectionNote, { color: colors.textSecondary }]}>
        Dados que o membro compartilha no perfil público.
      </Text>
